...
#represents a train line consisting of stations, neighboring lines and, of course, a name.
class Line

  attr_reader :stations # Array of String objects (the names of the stations on this line)
  attr_reader :name # String
  attr_accessor :neighbour_lines # Array of Line objects ( other Line's that happen to dissect this Line )
  attr_accessor :junction_stations # Array of String objects (the names of the stations on this line that are junctions, stations that people can change train lines at)
...
#provides functionality to determine the route from an 'origin' station to a 'dest'ination station
class RouteFinder

  def initialize(lines)
    @lines = lines # an array of all the train network's Line's
    @lines_searched = [] # array to keep track of lines that have already been searched when determining path from origin to destination.
  end
  ...
  def get_route(origin,dest)
    ...
    @lines_searched = []
    #then get the route, and interpret it to add in the line hops, and then get it in a readable form.
    get_readable_route(interpret_route(get_route_recurse(origin,dest)))
  end

  #helper method to get an array of lines that a particular station lies on - will only have one line unless the station is a junction.
  ...
  private
    ...
    #this is the recursive function that finds the path from origin to destination. It takes two stations as arguments, an origin station and a destination station and then, looking on the origin's line, checks if the destination station is on that line too. If it is it returns the route from station to station, if it isn't then it calls itself again but with the origin changed to be one of the junction stations on the origin's line and the process repeats but now from the point of view of the junction being the origin, it repeats until either the origin becomes a junction that is on a line that the destination station is also on, or the destination wasn't found in which case the function returns nil.
    # it keeps a list of lines it has previously checked so that if it exhausts its lines to search and hasn't yet found the destination station that it doesn't begin going back the way it came, that 'branch' is abandoned as a deadend by it returning nil. it then handles a return of nil as an indication that the destination is not in that direction. Eventually one of the paths will find the destination station and at that point the function will stop calling itself and will start returning solid results back to earlier versions of itself all the way back to the intial 'version' of this function which then returns the value to the original calling function (def get_route)
    def get_route_recurse(origin,dest)      
      ...
      origins_lines.each do |origin_line|
        ...
        if !common_line.nil? # if we've already found a common line no need to make more comparisons so stop looking:
          break
        end
      end
      
      # if the stations didn't share a common line, then we begin the recursive process:
      # but if they did, then we can simply list the stations between them and return the actual route (this will also be what terminates the recursive process)
      ...
      if common_line.nil?  # (then the stations are not on the same line)
        ...
        # then determine neighbouring lines that have not already been searched,
        ...
        if neighbouring_lines_to_search.empty?
          return nil  # then we've reached a point where we haven't found the destination station in this direction.
        else
          ...
          neighbouring_lines_to_search.each do |neighbour_line_to_search|
            ...
            neighbour_line_to_search.junction_stations.each do |junction_station|
              ...
              if stations_on_same_line?(origin, junction_station)
                # ensure we are looking at another junction if we're already 'on' one:
                unless origin == junction_station
                  #calculate the route from our 'current' origin to the junction station we're considering
                  route_from_origin_to_junction = get_route_recurse(origin, junction_station) 
                  #also calculate the route from the junction station we're considering to the destination station:
                  route_from_junction_to_dest = get_route_recurse(junction_station, dest)
                  #if either of those return nil then it means we were not considering an appropriate junction station or we have been looking in the wrong direction to get to our destination station and have hit a dead-end
                  if !route_from_origin_to_junction.nil? && !route_from_junction_to_dest.nil?
                    #then so far, we're on a branch that's either worth pursuing further, or we have already found the destination station.
                    return route_from_origin_to_junction + route_from_junction_to_dest
                  end
                else
                  return nil # call back up the stack that this branch was not worth pursuing
                end
              end
            end
          end
        ...
        # this block of code is basically the termination clause of our recursion.
        #it means that the 'current' origin we're looking from can see the destination station on one of the lines that it's also on:
        origin_index_in_line = common_line.stations.index(origin)
        dest_index_in_line = common_line.stations.index(dest)
        route = nil
        #ensure the order of the stations reflects the direction we're travelling:
...
#this represents a line coordinator person that looks at a map and determines which stations are junction stations.
class LineManager
  ...
  def determine_neighbours_and_junctions(array_of_lines)
    ...
    array_of_lines.each_with_index do |line, line_index|
      ...
      line.stations.each do |station|
        # and check if the station is also in any of the other lines. by checking every line
        array_of_lines.each_with_index do |other_line, other_lines_index|
          ...
          unless other_lines_index == line_index
            ...
            if other_line.contains_station?(station)
              # and if so, adding it to this line's list of neighbours and adding the station to the list of stations that are a junction between two lines.
        ...
        # if a junction is a junction of more than two lines, it will have been duplicated by the above code so remove duplicates as a precaution:
...
lm = LineManager.new
#get a line manager to analyse the lines to determine which lines are neighbours and which stations are junctions:
